...
public class Main1Activity extends BaseActivity implements View.OnClickListener {
    ...
    private void showNewRecordDialog(@Nullable final List<RecordListBean.RecordBean> recordBeanList) {
        NewRecordDialog.Builder builder = new NewRecordDialog.Builder(Main1Activity.this, true, new DialogInterface.OnCancelListener() {
            @Override
    ...
    private void initView() {
        ...
        if (btnPause != null) {
            btnPause.setOnClickListener(this); // 添加点击事件监听器
        } else {
            // 如果btnPause为空，显示一条日志消息，指示未找到btn_pause视图
            Log.e(TAG, "Button Pause not found in layout");
        }
    ...
    public void onClick(View v) {
        ...
        } else if (viewId == R.id.btn_record_list) {
            if (isStart) {
                Intent intent = new Intent(this, RecordListActivity.class);
                startActivity(intent);
            }
        } else if (viewId == R.id.btn_restart) {
            if (isStart) {
                handler.sendEmptyMessage(RESTART);
            }
        } else if (viewId == R.id.btn_space) {
